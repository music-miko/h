...
# V2 SETTINGS
# -----------------------
# per HTTP request retries (network/timeouts/5xx)
V2_HTTP_RETRIES = 2

# full-flow restart count (if 2 => run the whole flow twice max)
...
async def v2_download(link: str, media_type: str) -> Optional[str]:
    ...
    for cycle in range(1, V2_DOWNLOAD_CYCLES + 1):
        try:
            ...
            # hard fail: stop completely (no more cycles)
            return None
