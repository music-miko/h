...
#   VIDEO_API_URL -> just a toggle for enabling video API usage
...
# =======================================================================
# Deadlinetech.site API integration for merged_api.py /song endpoint
# =======================================================================
...
async def _deadlinetech_download(link: str, media_type: str) -> Optional[str]:
    ...
    merged_api.py returns JSON like:
        {
          "status": "done",
          "video_id": "...",
          "type": "audio" | "video",
          "file_path": "/abs/path" | "downloads/ID.ext" | "http(s)://...",
          "ext": "m4a" | "mp4" | ...
          ...
        }

    We then:
      - If file_path is HTTP(S) → download and return local path.
      - Else if it's a local path that exists → use directly.
      - Else treat it as relative on Deadlinetech and download from:
            {API_URL}/{file_path}
    """
    ...
    try:
        ...
        async with session.get(url, params=params) as resp:
            if resp.status != 200:
                ...
                LOGGER.warning(
                    f"Deadlinetech API returned HTTP {resp.status} for {vid}. Body: {text[:200]}"
                )
                return None

            content_type = (resp.headers.get("Content-Type") or "").lower()

            # Expect JSON (from merged_api)
            if "application/json" in content_type or "text/json" in content_type:
                ...
                if str(file_path).startswith(("http://", "https://")):
                    ext = (
                        data.get("ext")
                        or os.path.splitext(str(file_path))[1].lstrip(".")
                    ...
                    return await download_file(str(file_path), out_path)

                # Otherwise: local/relative path from API server
                local_path = str(file_path)

                # 1) If absolute and exists on this machine, use it
                if os.path.isabs(local_path) and os.path.exists(local_path):
                    return local_path

                # 2) If same basename already exists in our DOWNLOAD_DIR, use it
                candidate = os.path.join(DOWNLOAD_DIR, os.path.basename(local_path))
                if os.path.exists(candidate):
                    return candidate

                # 3) Otherwise, treat as path exposed by Deadlinetech via HTTP
                #    e.g. "downloads/HK1ICIPt4Kg.webm" -> https://deadlinetech.site/downloads/HK1ICIPt4Kg.webm
            ...
            # Fallback: Deadlinetech directly streams binary (not typical with merged_api, but safe)
...
async def api_download_video(link: str) -> Optional[str]:
    ...
    VIDEO_API_URL just toggles whether video API is allowed.
    """
