...
# V2 SETTINGS
# -----------------------
# retry only network/timeouts/5xx inside a single HTTP call
V2_HTTP_RETRIES = 2

# full flow cycles (your ask: if retries number is 2 -> try whole flow again)
...
# when "no_candidate" happens, wait a bit before next cycle (helps jobs finish)
NO_CANDIDATE_WAIT = 5

# CDN retry (fixes your "first fail second success" issue)
CDN_RETRIES = 2
CDN_RETRY_DELAY = 2
...
def _is_hard_status(status: int) -> bool:
    return status in (401, 403)


def _extract_candidate(obj: Any) -> Optional[str]:
    """
    Extracts best candidate URL from arbitrary API JSON shapes.
    """
    if obj is None:
        return None
    ...
    if isinstance(obj, dict):
        # job -> result -> public_url/cdnurl/etc
        job = obj.get("job")
...
async def _download_from_cdn(cdn_url: str, out_path: str) -> Optional[str]:
    """
    Retries CDN for transient failures (timeout/network, 5xx, 429).
    Does NOT spam logs. Stats are tracked.
    """
    if not cdn_url:
        return None
    ...
    for attempt in range(1, V2_HTTP_RETRIES + 1):
        try:
            ...
            async with session.get(
                ...
                if _is_hard_status(resp.status):
                    if resp.status == 401:
                        _inc("hard_fail_401")
                    else:
                        _inc("hard_fail_403")
...
async def v2_download(link: str, media_type: str) -> Optional[str]:
    ...
    Improvements:
      - CDN retry (transient)
      - no_candidate waits before next cycle
      - hard 401/403 stops immediately
    """
    ...
    for cycle in range(1, V2_DOWNLOAD_CYCLES + 1):
        ...
        # No candidate: wait and retry cycle
        if not candidate:
            _inc("no_candidate")
            if cycle < V2_DOWNLOAD_CYCLES:
...
async def media_download(link: str, type: str, title: str = "") -> Optional[str]:
    """
    V2 ONLY. Logs ONLY final success/fail. Stats are in DOWNLOAD_STATS.
    """
    _inc("total")
