...
async def _deadlinetech_download(link: str, media_type: str) -> Optional[str]:
    ...
    We ALWAYS:
      - Turn that into a remote URL:
          * if file_path is HTTP(S) -> use it directly
          * else -> {API_URL}/{file_path.lstrip('/')}
      - Download it to our bot's DOWNLOAD_DIR as {video_id}.{ext}
      - Return that LOCAL path for playback.
    """
        ...
        async with session.get(url, params=params) as resp:
            ...
            if "application/json" in content_type or "text/json" in content_type:
                ...
                if file_path_str.startswith(("http://", "https://")):
                    remote_url = file_path_str
                else:
                    # ALWAYS treat as HTTP path on Deadlinetech
                    # e.g. "downloads/ID.webm" -> https://deadlinetech.site/downloads/ID.webm
                    remote_url = f"{base}/{file_path_str.lstrip('/')}"

                # Decide extension
                ext = (
                    data.get("ext")
                    or os.path.splitext(file_path_str)[1].lstrip(".")
                ...
                LOGGER.info(
                    f"Deadlinetech JSON file_path -> downloading from {remote_url} to {out_path}"
                )
